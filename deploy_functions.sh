...
deploy_function() {
    ...
    if [ "${SCHEDULED_JOB_NAME}" == "metoffice" ]; then
        ...
        local SOURCE="/Users/jamieconn/isthedartrunning/functions/metoffice"
        local TIMEOUT=300
        local MEMORY=256M
    
    elif [ "${SCHEDULED_JOB_NAME}" == "dartcom" ]; then
        local FUNCTION_NAME="update_rainfall"
        local SOURCE="/Users/jamieconn/isthedartrunning/functions/dartcom"
        local TIMEOUT=60
        local MEMORY=256M

    elif [ "${SCHEDULED_JOB_NAME}" == "env_agency" ]; then
        local FUNCTION_NAME="update_level"
        local SOURCE="/Users/jamieconn/isthedartrunning/functions/env_agency"
        local TIMEOUT=60
        local MEMORY=256M

     elif [ "${SCHEDULED_JOB_NAME}" == "model" ]; then
        local FUNCTION_NAME="run_model"
        local SOURCE="/Users/jamieconn/isthedartrunning/functions/model"
        local TIMEOUT=240
        local MEMORY=2048
    
    elif [ "${SCHEDULED_JOB_NAME}" == "api" ]; then
        local FUNCTION_NAME="fetch_data"
        local SOURCE="/Users/jamieconn/isthedartrunning/functions/api"
        local TIMEOUT=240
        local MEMORY=2048
    else
        return
